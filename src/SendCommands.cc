...
void send_server_time(std::shared_ptr<Client> c) {
  ...
  struct tm t_parsed;
  // Must be UTC: the client turns this into the "internet time" @beat (Swatch time), which is
  // UTC-based and timezone-less by design. Sending local time would skew the @beat.
